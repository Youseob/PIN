...
class PromptedClassificationReward(BaseReward):
    def __init__(
        ...
        if self.is_mask_lm:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(self.task_lm)
            ...
            self._generator = (GPT2LMHeadModel
                               .from_pretrained(self.task_lm)
                               .to(self.device))
            self._generator.config.pad_token_id = self.tokenizer.pad_token_id
        ...
        self.verbalizer_ids = [self.tokenizer.convert_tokens_to_ids(v)
    ...
    def _get_logits(
        ...
        encoded_inputs = self.tokenizer(texts, padding='longest',
