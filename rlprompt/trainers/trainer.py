...
class Trainer:
    ...
    def train(self,
        ...
        if report_to_wandb:
            ...
            max_result_table = wandb.Table(columns=["epoch", "max_score", "prompt"])
        ...
        for epoch in range(total_train_epochs):
            for batch in train_dataloader:
                ...
                if self.do_eval and eval_by_steps \
                    ...
                    eval_prompt, eval_log = self.evaluate()
                    text_table.add_data(epoch, round(eval_log['eval/score'], 2), eval_prompt)
                    if eval_log['eval/score'] > max_score:
                        max_score = eval_log['eval/score']
                        max_eval_prompt = eval_prompt
                    max_result_table.add_data(epoch, max_score, max_eval_prompt)
                    if report_to_wandb:
                        wandb.log(eval_log)
            ...
            if self.do_eval and not eval_by_steps:
                ...
                eval_prompt, eval_log = self.evaluate()
                text_table.add_data(epoch, round(eval_log['eval/score'], 3), eval_prompt)
                if eval_log['eval/score'] > max_score:
                   max_score = eval_log['eval/score']
                   max_eval_prompt = eval_prompt
                max_result_table.add_data(epoch, max_score, max_eval_prompt)
        ...
        run.log({"val_prompt": max_result_table})
        df = text_table.get_dataframe()
        df_new = df.sort_values(by=['score'], ascending=False).reset_index(drop=True)
        csv_dir = os.path.join(self.save_dir, f'{self.num_exp}_prompt.csv')
        df_new.to_csv(csv_dir, encoding='utf-8')    
        wandb.finish() 
    ...
    def evaluate(
        ...
        print('Finish Eval')
        prompt = self.module._reward.tokenizer.convert_tokens_to_string(hypos[-1])
        return prompt, \
