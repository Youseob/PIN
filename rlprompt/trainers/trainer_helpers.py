...
from rlprompt.trainers import Trainer, Trainer_for_clip
...
def make_trainer_for_clip(module: BaseModule, config: "DictConfig") -> Trainer:
    return Trainer_for_clip(module, config.num_train_epochs, config.max_train_steps, 
                   config.do_eval, config.eval_batch_size, 
                   config.eval_steps, config.do_save, config.save_dir, 
                   config.save_steps, config.learning_rate, 
                   config.gradient_clip, config.gradient_clip_norm, 
                   config.checkpoint_path, config.random_seed,
