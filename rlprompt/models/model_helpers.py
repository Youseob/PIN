...
from rlprompt.models import BaseModel, LMAdaptorModel, SinglePromptModel, InputConditionedPromptModel, LMFilterModel, LM_Model, LM_SPARSE_Model

def make_lm_model(config: "DictConfig"):
    if config.algo == 'sql':
        return LM_Model(config.policy_lm,
                        config.hidden_size,
                        config.logit_bias,
                        config.fluent,
                        config.fluent_top_k,
        ...
        return LM_SPARSE_Model(config.policy_lm,
                               config.hidden_size,
                               config.logit_bias,
                               config.fluent,
                               config.fluent_top_k,
...
def make_lm_adaptor_model(config: "DictConfig"):
    if config.algo == "sql":
        ...
        return LMFilterModel(config.policy_lm,
                            config.hidden_size,
                            config.logit_bias,
                            config.fluent,
                            config.fluent_top_k,
